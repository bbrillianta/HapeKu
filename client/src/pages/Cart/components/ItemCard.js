...
const ItemCard = ({item , handleDelete , changeTotal , setChangeTotal}) => {
    ...
    const handlePlusFunction = (itemId , productId) => {
        let tambahan = quantity + 1;
    ...
    const handleMinusFunction = (itemId , productId) => {
        let kurangan = quantity - 1;
        if (kurangan >= 1) {
            axios.put('http://localhost:3001/user/cart/' , {
                itemId: itemId,
                product: productId,
    ...
    return (
        <div>
            <CardProduct key={item._id}>
                <div style={{display: 'flex'}}>
                    <img 
                        src={`http://localhost:3001/${item.product.thumbnail.path}`} 
                    ...
                    <div style={styles.action}>
                        <div className="opsi"
                            ...
                            <OpsiButton onClick={() => handleMinusFunction(item._id , item.product._id)}>-</OpsiButton>
                            <span>{quantity}</span>
                            <OpsiButton onClick={() => handlePlusFunction(item._id , item.product._id)}>+</OpsiButton>
                            <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{cursor: 'pointer'}} onClick={() => handleDeleteFunction(item._id)} >
                ...
                <div>
                    <div className="subtotal" style={styles.flex}>
                        ...
                        <h4>
                            {
                                convertToRupiah(quantity * item.product.price)
                            }
                        </h4>
                    </div>
                    <div className="harga" style={styles.flex}>
                        <p style={{fontWeight: 'lighter', color: 'grey'}}>Harga</p>
                        <p style={{fontWeight: 'lighter', color: 'grey'}}>{convertToRupiah(quantity * item.product.price)}</p>
                    </div>
                    <div className="jumlah" style={styles.flex}>
                        <p style={{fontWeight: 'lighter', color: 'grey'}}>Jumlah Barang</p>
                        <p style={{fontWeight: 'lighter', color: 'grey'}}>{quantity}</p>
                    </div>
                </div>
            </CardProduct>
        </div>
